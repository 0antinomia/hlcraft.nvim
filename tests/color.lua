...
local nan_channel_ok = pcall(color.clamp_channel, 0 / 0)
h.assert_true(not nan_channel_ok, 'RGB channel accepted NaN', scope)
local string_channel_ok = pcall(color.clamp_channel, '128')
h.assert_true(not string_channel_ok, 'RGB channel accepted a string', scope)
local invalid_rgb_hex_ok = pcall(color.rgb_to_hex, 0, 0 / 0, 0)
h.assert_true(not invalid_rgb_hex_ok, 'RGB hex conversion accepted NaN', scope)
